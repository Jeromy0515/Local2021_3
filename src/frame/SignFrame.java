	...
	private void okBtnAct() {
		...
		if(JOptionPane.showConfirmDialog(null, "청접장을 보내겠습니까?","정보",JOptionPane.YES_NO_OPTION) == 0) {
			try (PreparedStatement pst = conn.prepareStatement("insert into payment values (?,?,?,?,?,?,?,?)")){
				...
				pst.setObject(2, getPmk("select * from weddinghall where wh_Name = '"+ui.whName+"'", "wh_No"));
				pst.setObject(3, ui.people);
				pst.setObject(4, getPmk("select * from weddingtype where wty_Name = '"+ui.whType+"'", "wty_No"));
				pst.setObject(5, getPmk("select * from mealType where m_Name = '"+ui.mealType+"'", "m_No"));
				pst.setObject(6, ui.ivNum);
				pst.setObject(7, ui.date);
				pst.setObject(8, user_No);
				pst.execute();
				
				openFrame(new FriendListFrame());
			} catch (Exception e) {
				e.printStackTrace();
			}
		}else {
	 ...
	 int getPmk(String sql,String col){
